...
def ask_openai_with_context(user_query, context, chat_history, openai_api_key):
    system_prompt = f"""
     "You are a helpful assistant.answer the user query according  to the content given."
    "Use the following context to answer the user's question. If the answer is available in the context, provide it."
    "Only provide answers that can be derived from the document content. If unsure, say that the information is not available."
    "Do not assume or generate answers beyond the given content.do not generate any harmful or abusive respones."
    Context : (
    {context})
    """
...
def main():
    ...
    #             headers = {"Api-Key": pinecone_key_input.strip()}
    #             response = requests.get("https://api.pinecone.io/whoami", headers=headers)

    #             if response.status_code == 200:
    #                 st.session_state["pinecone_api_key"] = pinecone_key_input.strip()
    #                 st.success("Pinecone API Key is valid! 🎉")
    #                 st.rerun()
    #             else:
    ...
    #     st.stop()
    # else:
    #     st.success("Pinecone API Key already set! You can proceed. 🚀")
    ...
    if uploaded_file is not None and not st.session_state["file_processed"]:
        user_id = st.session_state["user_id"]  # however you get your user ID
